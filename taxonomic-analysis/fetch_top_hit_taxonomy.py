"""
Script name:
    fetch_top_hit_taxonomy.py

Purpose:
    Fetch taxonomy information for the best-scoring homolog detected for each
    strain-specific Vibrio natriegens query gene in MMseqs2 .m8 result files.

Description:
    This script reads MMseqs2 `.m8` result files, extracts the query, target, and
    bitscore columns, and keeps only the highest-bitscore hit for each query gene.
    It then retrieves TaxID information for the target proteins using the UniProtKB
    REST API and retrieves taxonomic lineage information using NCBI Entrez Taxonomy,
    with an NCBI Datasets fallback.

Input:
    - MMseqs2 `.m8` result files.

Output:
    - Taxonomy-annotated TSV file for the best-scoring hit per query gene.
...
out_dir = f"/path/to/output/top_hits_taxonomy/{dataset_name}"
os.makedirs(out_dir, exist_ok=True)

# Shared cache (same file used by all domains)
...
#Strip strain-level descriptors to keep canonical species name
...
# Read mmseqs2 result files
def read_top_hits(folder: str) -> pd.DataFrame:
    ...
    for f in tqdm(files, desc="Reading .m8 files", unit="file"):
        ...
        df = pd.read_csv(path, sep="\t", header=None, usecols=[0, 1, 11],
                         names=["query", "target", "bitscore"],
                         dtype={"query": str, "target": str, "bitscore": float})
        frames.append(df)

    df = pd.concat(frames, ignore_index=True)
    df = df.sort_values("bitscore", ascending=False).drop_duplicates(subset="query", keep="first")
    df = df.drop(columns=["bitscore"])  # Remove bitscore column before output
    df["target"] = df["target"].map(canonicalize)
    return df
    ...
    for i in tqdm(range(0, len(accs), UNIPROT_BATCH), desc="Fetching TaxIDs (UniProt)", unit="batch"):
        ...
        if "Entry" in df.columns and "Organism (ID)" in df.columns:
            for _, row in df.iterrows():
                acc = str(row["Entry"]).strip()
                taxid = str(row["Organism (ID)"]).strip()
                if acc and taxid and taxid != "nan":
                    result[acc] = taxid
        time.sleep(PAUSE)
    return result
...
# Main workflow
def main():
    print(f"\n--- Taxonomy: TOP hits | dataset={dataset_name} ---\n")

    df = read_top_hits(results_dir)
    ...
    out_tsv = os.path.join(out_dir, f"{dataset_name}_top_hits_taxonomy.tsv")
    df.to_csv(out_tsv, sep="\t", index=False)

    report = os.path.join(out_dir, f"{dataset_name}_top_hits_taxonomy_report.txt")
